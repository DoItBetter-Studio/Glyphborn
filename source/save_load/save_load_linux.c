...
// ---------------------------------------------------------------------
// Path resolution -- $XDG_DATA_HOME/glyphborn/saves/ or ~/.local/share/...
// ---------------------------------------------------------------------

// Creates each component of `path` in turn (mkdir -p). `path` must already
// be null-terminated; a trailing separator is fine.
static void mkdir_recursive(const char* path)
{
	...
	for (size_t i = 1; i < len; i++)
	{
		...
		{
			tmp[i] = '\0';
			mkdir(tmp, 0755); // ignore EEXIST and other errors -- best effort
			tmp[i] = '/';
		}
	}
...
bool save_get_directory(char* out, size_t out_size)
{
	...
	if (xdg && xdg[0])
		written = snprintf(out, out_size, "%s/glyphborn/saves/", xdg);
	else
	{
		...
		written = snprintf(out, out_size, "%s/.local/share/glyphborn/saves/", home);
	}
