...
namespace Glyphborn.Mapper.Controls
{
	public sealed class MapCanvasControl : Control
	{
		...
		private void DrawTile(Graphics g, MapDocument srcMap, int sx, int sy, int tileSize, int ox, int oy, int layer, float alpha = 1.0f)
		{
			...
			// compute screen x/y using tx/sy relative to current map origin.
			// The earlier OnPaint loop used tx/ty for position; here we deduce position from sx and the intended tx:
			// Simpler: calculate screen position using the tile's logical position passed by caller:
			// We'll compute px/py via the caller's tx/ty logic by reversing: (ox + tx * tileSize).
			// But we don't have tx here — compute tile screen position from sx and map offsets by comparing
			// neighbor offsets to the active map. Simpler approach: compute screen coordinates based on sx/ sy and
			// relative map offsets:
			int mapOffsetX = 0;
			int mapOffsetY = 0;
